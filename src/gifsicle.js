...
const gifsicle = (function () {
  const wasmUrl = "gifsicle.wasm";
  const FS_BAK = null;
  const gifsicle_c = (function () {
    var _scriptDir =
    ...
    return function (gifsicle) {
          ...
          stream_ops.read = function stream_ops_read(stream, buffer, offset, length, position) {
            ...
            if (contents.slice) {
              ...
              for (var j = 0; j < size; j++) {
                buffer[offset + j] = contents.get(position + j);
              }
            }
            return size;
          };
      ...
      var ___wasm_call_ctors = (Module["___wasm_call_ctors"] = function () {
        return (___wasm_call_ctors = Module["___wasm_call_ctors"] = Module["asm"]["w"]).apply(
          null,
          arguments
        );
      });
